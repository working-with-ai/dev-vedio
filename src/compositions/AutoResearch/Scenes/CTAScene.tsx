...
export const CTAScene: React.FC<AutoResearchProps> = ({
  backgroundColor,
  accentColor,
  highlightColor,
  ctaLine1,
  ...
  const statsStart = Math.round(fps * 3);
  ...
  const starCountAnim = numberCountUp(frame, fps, githubStars / 1000, 1.5, 5);
  const pulseScale = 1 + Math.sin(frame / fps * Math.PI * 3) * 0.02;

  const platforms = [
    { name: "Claude", icon: "🟠", color: "#f97316" },
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          >
            {[
              { value: `${(starCountAnim / 10).toFixed(1)}k`, label: "Stars", color: "#fbbf24" },
              { value: `${codeLines}`, label: "Lines", color: accentColor },
            ].map((s, i) => (
              <div key={i} style={{ textAlign: "center" }}>
                ...
                <div style={{ fontSize: 18, color: "#666", letterSpacing: 4 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ textAlign: "center", fontSize: 18, color: "#666", letterSpacing: 5, marginBottom: 12, opacity: fadeIn(frame, statsStart, 15) }}>
            COMPATIBLE AGENTS
          </div>
          <div style={{ display: "flex", justifyContent: "center", gap: 12, flexWrap: "wrap" }}>
            {platforms.map((tool, i) => {
              const anim = cardSlideIn(frame, fps, statsStart + staggerDelay(i, 6));
              return (
                <div
                  key={i}
                  style={{
        ...
        >
          #AutoResearch #Karpathy #AI编程 #GitHub
        </div>
      </div>
    </SceneBackground>
  );
