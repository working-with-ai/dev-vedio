...
const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "pencildev";
const defaultScripts = PencilDevSchema.parse({}).voiceoverScripts;
...
/** Ensure each word starts after the previous word ends; fix end < start. */
function enforceMonotonicWords(words: SubtitleWord[]): SubtitleWord[] {
  const out: SubtitleWord[] = [];
  let prevEnd = Number.NEGATIVE_INFINITY;

  for (const w of words) {
    let startFrame = Math.max(w.startFrame, prevEnd);
    let endFrame = Math.max(w.endFrame, startFrame);
...
function buildWords(text: string, startFrame: number, endFrame: number): SubtitleWord[] {
  ...
  const visibleChars = chars.filter((char) => !/[\s，。！？、：；,.!?]/.test(char)).length || chars.length;
...
async function syncSubtitles() {
  console.log("🔄 开始同步 PencilDev 字幕和配音...\n");
  ...
  console.log(`\n✨ PencilDev 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`);
}
