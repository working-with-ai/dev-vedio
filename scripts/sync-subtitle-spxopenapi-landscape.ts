...
const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "spxopenapi-landscape";
...
function buildWords(
  ...
  const visibleChars =
    chars.filter((char) => !/[\s，。！？、：；,.!?]/.test(char)).length ||
    chars.length;
...
async function syncSubtitles() {
  console.log("🔄 开始同步 SPXOpenAPILandscape 字幕和配音...\n");
  ...
  for (let index = 0; index < SCENE_COUNT; index++) {
    const audioFile = path.join(
      audioDir,
      `${COMPOSITION_PREFIX}-scene${index + 1}.mp3`,
    );
    if (fs.existsSync(audioFile)) {
      const duration = await getAudioDuration(audioFile);
      console.log(
        `📊 场景 ${index + 1}: ${duration.toFixed(2)}秒 (${Math.round(duration * fps)}帧)`,
      );
      audioInfos.push({
  ...
  for (let index = 0; index < audioInfos.length; index++) {
    ...
    const sceneDuration = Math.max(
      minSceneDuration,
      audioInfo.durationFrames + buffer,
    );
    const sceneStartFrame = currentFrame;
    const audioStartFrame = sceneStartFrame + sceneDelay;
    const vttFile = path.join(
      audioDir,
      `${COMPOSITION_PREFIX}-scene${index + 1}.vtt`,
    );
    ...
    if (fs.existsSync(vttFile)) {
      ...
      cues.forEach((cue) => {
        const cueStartFrame =
          audioStartFrame + Math.round((cue.startMs / 1000) * fps);
        let cueEndFrame =
          audioStartFrame + Math.round((cue.endMs / 1000) * fps);
  ...
  console.log(
    `\n✨ SPXOpenAPILandscape 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`,
  );
}
