...
import { HermesAgentSchema } from "../src/compositions/HermesAgent/schema";

const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "hermesagent";
const defaultScripts = HermesAgentSchema.parse({}).voiceoverScripts;
...
async function syncSubtitles() {
  console.log("🔄 开始同步 HermesAgent 字幕和配音...\n");
  ...
  console.log(`\n✨ HermesAgent 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`);
}
