...
import { AIHarnessEngineerSchema } from "../src/compositions/AIHarnessEngineer/schema";

const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "aiharnessengineer";
const defaultScripts = AIHarnessEngineerSchema.parse({}).voiceoverScripts;
...
function enforceMonotonicWords(words: SubtitleWord[]): SubtitleWord[] {
  ...
  for (const word of words) {
    let startFrame = Math.max(word.startFrame, prevEnd);
...
async function syncSubtitles() {
  console.log("🔄 开始同步 AIHarnessEngineer 字幕和配音...\n");
  ...
  console.log(`\n✨ AIHarnessEngineer 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`);
}
