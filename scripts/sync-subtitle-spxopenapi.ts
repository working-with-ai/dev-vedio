...
import { SPXOpenAPISchema } from "../src/compositions/SPXOpenAPI/schema";

const SCENE_COUNT = 7;
const COMPOSITION_PREFIX = "spxopenapi";
const defaultScripts = SPXOpenAPISchema.parse({}).voiceoverScripts;
...
async function syncSubtitles() {
  console.log("🔄 开始同步 SPXOpenAPI 字幕和配音...\n");
  ...
  console.log(`\n✨ SPXOpenAPI 字幕同步完成，总时长 ${(totalDuration / fps).toFixed(1)} 秒`);
}
